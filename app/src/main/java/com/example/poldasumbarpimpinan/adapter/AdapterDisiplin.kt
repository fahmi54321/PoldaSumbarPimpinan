...
import kotlinx.android.synthetic.main.item_get_disiplin.view.*
import java.util.*

class AdapterDisiplin
    (
        var data:List<ResultItemDisiplin?>?,
        var context: Context
):RecyclerView.Adapter<AdapterDisiplin.DisiplinViewHolder>() {
    class DisiplinViewHolder(itemView:View):RecyclerView.ViewHolder(itemView) {
        ...
        var id_disiplin = itemView.id_disiplin
        var buttonDetails = itemView.buttonDetails
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): DisiplinViewHolder {
        var view = LayoutInflater.from(parent.context).inflate(R.layout.item_get_disiplin,parent,false)
        var holder =
            DisiplinViewHolder(view)
        return holder
    }
    ...
    override fun onBindViewHolder(holder: DisiplinViewHolder, position: Int) {
        ...
        holder.id_disiplin.text = data?.get(position)?.idDisiplin
